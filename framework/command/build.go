...
var buildSelfCommand = &cobra.Command{
	...
	RunE: func(c *cobra.Command, args []string) error {
		...
		if err != nil {
			log.Fatalln("goweb go: please install go in path first")
		}
		...
		fmt.Println("FILENAME: " + execName)
		cmd := exec.Command(path, "build", "-o", execName, "./")
		...
		fmt.Println("build success please run ./goweb direct")
		return nil
	},
}
